...
SCENARIO_LABELS = {
    "baseline_ev_biogenesis": "Reference",
    "rab_escrt_limited": "Rab/ESCRT limited",
    "ceramide_secretory_bias": "Ceramide favored",
    "injury_apoptotic_shift": "Injury dominant",
}
        ...
        summary_rows.append(
            {
                ...
                "peak_apoptotic_commitment": result.summary[
                    "peak_apoptotic_commitment"
                ],
...
def write_outputs(
    ...
    STANDARD_ABBREVIATIONS.rename_columns(summary).to_csv(
        outdir / "ev_biogenesis_summary.csv", index=False
    )
    ...
    STANDARD_ABBREVIATIONS.write_bundle(
        outdir, keys=("EV", "MVB", "ILV", "ESCRT", "sEV", "m/lEV", "AB")
    )
    if make_plots:
...
def _plot_timeseries(timeseries: pd.DataFrame, outdir: Path) -> None:
    ...
    with manuscript_style_context():
        ...
        for index, (scenario, frame) in enumerate(
            ...
            axes[0, 0].plot(frame["t"], frame["MVB_pool"], label=label, **style)
            axes[0, 1].plot(frame["t"], frame["secretory_bias"], label=label, **style)
            axes[1, 0].plot(frame["t"], frame["sEV_rate"], label=label, **style)
            axes[1, 1].plot(frame["t"], frame["AB_rate"], label=label, **style)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", type=Path, default=Path("results/ev_biogenesis_comparison")
    )
    parser.add_argument(
